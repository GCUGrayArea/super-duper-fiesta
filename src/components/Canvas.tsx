  ...
  useEffect(() => {
    if (canvasRef.current && !fabricCanvasRef.current) {
      ...
      canvas.on('mouse:down', (opt) => {
        ...
        // Start panning if Alt key is held OR if clicking on empty canvas space
        if (evt.altKey === true || !opt.target) {
  ...
  return (
    <div className="canvas-container">
      ...
      <div className="canvas-controls">
        <p>Drag empty space to pan • Hold Alt + drag anywhere to pan • Scroll to zoom • Double-click text to edit</p>
      </div>
    </div>
  );
};
